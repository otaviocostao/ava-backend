...
export class ClassesService {
  ...
  async create(createClassDto: CreateClassDto): Promise<Class> {
    ...
    return this.classRepository.findOne({
      ...
      relations: ['discipline', 'teacher', 'academicPeriod', 'schedules'],
    }) as Promise<Class>;
  }
...
//Atualizar Classe
  async update(id: string, updateClassDto: UpdateClassDto): Promise<Class> {
    ...
    if (!classEntity) {
      throw new NotFoundException(`Classe com o ID '${id}' nao encontrada.`);
    }

    let academicPeriod: AcademicPeriod | null = classEntity.academicPeriod ?? null;

    if (disciplineId) {
      const discipline = await this.disciplineRepository.findOneBy({ id: disciplineId });
      if (!discipline) {
        throw new NotFoundException(`Disciplina com ID "${disciplineId}" nao encontrada.`);
      }
      classEntity.discipline = discipline;
    }

    if (academicPeriodId) {
      const foundAcademicPeriod = await this.academicPeriodRepository.findOneBy({ id: academicPeriodId });
      if (!foundAcademicPeriod) {
        throw new NotFoundException(`Periodo letivo com ID "${academicPeriodId}" nao encontrado.`);
      }
      academicPeriod = foundAcademicPeriod;
      classEntity.academicPeriod = foundAcademicPeriod;
    }

    if (teacherId) {
      const teacher = await this.userRepository.findOneBy({ id: teacherId });
      if (!teacher) {
        throw new NotFoundException(`Usuario com ID "${teacherId}" nao encontrado.`);
      }
    ...
    const shouldHandleSchedule = [dayOfWeek, startTime, endTime, room].some(
    ...
    if (shouldHandleSchedule) {
      ...
      if (existingSchedules.length > 0) {
        await this.schedulesService.update(existingSchedules[0].id, {
          ...(dayOfWeek && { dayOfWeek }),
          ...(startTime && { startTime }),
          ...(endTime && { endTime }),
          ...(room !== undefined ? { room } : {}),
        ...
        if (!dayOfWeek || !startTime || !endTime) {
          throw new BadRequestException(
            'Para criar o horario da turma, informe dayOfWeek, startTime e endTime.',
          );
        }

        if (!academicPeriod || !academicPeriod.startDate || !academicPeriod.endDate) {
          const classWithPeriod = await this.classRepository.findOne({
            where: { id },
            relations: ['academicPeriod'],
          });
          academicPeriod = classWithPeriod?.academicPeriod ?? null;
        }
        ...
        if (!academicPeriod.startDate || !academicPeriod.endDate) {
          throw new BadRequestException(
            `O periodo letivo "${academicPeriod.period}" nao possui datas de inicio e/ou fim definidas.`,
          );
        }
    ...
    return this.classRepository.findOne({
      where: { id: savedClass.id },
      relations: ['teacher', 'discipline', 'academicPeriod', 'schedules'],
    }) as Promise<Class>;
  }
